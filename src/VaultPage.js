...
const GBox = ({ children, style = {}, accent = C.purple }) => (
  <div style={{ background: C.panel, border: `1px solid ${accent}33`, borderRadius: 12, boxShadow: `0 0 40px ${accent}10`, ...style }}>{children}</div>
);
const Btn = ({ children, onClick, variant = "primary", style = {}, href }) => {
  if (href) return (
    <a href={href} download style={{
      background: `linear-gradient(135deg, ${C.purple}, ${C.teal})`,
      border: "none", borderRadius: 8, padding: "13px 28px", color: "#fff",
      fontSize: 13, cursor: "pointer", fontFamily: "'Courier New', monospace",
      fontWeight: 700, letterSpacing: 1.5, textTransform: "uppercase",
      boxShadow: `0 0 30px ${C.purple}44`, textDecoration: "none",
    ...
    <div style={{ background: C.bg, minHeight: "100vh", color: C.white }}>
      ...
      <section style={{ padding: "140px clamp(20px,6vw,80px) 80px", textAlign: "center", background: `linear-gradient(180deg, ${C.dark} 0%, ${C.bg} 100%)`, position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ maxWidth: 700, margin: "0 auto", position: "relative", zIndex: 1 }}>
          ...
          <p style={{ fontSize: 17, color: C.light, lineHeight: 1.8, maxWidth: 520, margin: "0 auto", fontFamily: "Georgia, serif" }}>
      ...
      <section style={{ padding: "60px clamp(20px,6vw,80px) 40px" }}>
        <div style={{ maxWidth: 1000, margin: "0 auto" }}>
          ...
          <GBox accent={C.amber} style={{ padding: "44px 44px", display: "grid", gridTemplateColumns: "1fr auto", gap: 48, alignItems: "center" }}>
            <div>
              <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 16 }}>
                <span style={{ fontSize: 28 }}>📋</span>
                <Pill text="Free Download" color={C.amber} />
              </div>
              ...
              <p style={{ fontSize: 15, color: C.light, lineHeight: 1.8, margin: "0 0 20px", fontFamily: "Georgia, serif", maxWidth: 520 }}>
              ...
              <div style={{ display: "flex", gap: 24, marginBottom: 24, flexWrap: "wrap" }}>
                {[["3 Pillars", "Data · Workflow · Decisions"], ["15 Questions", "Yes/No self-assessment"], ["5 Minutes", "Quick diagnostic tool"]].map(([num, label]) => (
                  <div key={num} style={{ textAlign: "center" }}>
                    ...
                    <div style={{ fontSize: 11, color: C.slate, fontFamily: "Georgia, serif" }}>{label}</div>
                  </div>
                ))}
              </div>
      ...
      {/* INFRASTRUCTURE FRICTION AUDIT */}
      <section style={{ padding: "40px clamp(20px,6vw,80px)" }}>
        <div style={{ maxWidth: 1000, margin: "0 auto" }}>
          <div style={{ marginBottom: 32 }}>
            <Pill text="Diagnostic Tool" color={C.teal} />
          </div>
          <GBox accent={C.teal} style={{ padding: "40px 44px", display: "grid", gridTemplateColumns: "1fr auto", gap: 40, alignItems: "center" }}>
            <div>
              <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 16 }}>
                <span style={{ fontSize: 24 }}>⚙️</span>
                <Pill text="Free Assessment" color={C.teal} />
              </div>
              <h2 style={{ fontSize: "clamp(18px,2.2vw,26px)", fontWeight: 900, color: C.white, margin: "0 0 12px", fontFamily: "Georgia, serif" }}>
                Operational Readiness Assessment
              </h2>
              <p style={{ fontSize: 15, color: C.light, lineHeight: 1.8, margin: "0 0 20px", fontFamily: "Georgia, serif", maxWidth: 500 }}>
            ...
            <div style={{ textAlign: "center", padding: 24 }}>
              <div style={{ fontSize: 48, fontWeight: 900, color: C.teal + "44", fontFamily: "'Courier New', monospace" }}>425</div>
              <div style={{ fontSize: 11, color: C.slate, letterSpacing: 2, textTransform: "uppercase", fontFamily: "'Courier New', monospace" }}>point system</div>
            </div>
          </GBox>
        </div>
      </section>
      ...
      <section style={{ padding: "60px clamp(20px,6vw,80px)", background: C.dark }}>
        <div style={{ maxWidth: 1000, margin: "0 auto" }}>
          <div style={{ marginBottom: 40 }}>
            ...
            <h2 style={{ fontSize: "clamp(22px,2.8vw,34px)", fontWeight: 900, margin: "16px 0 8px", fontFamily: "Georgia, serif" }}>
              Frameworks & Thinking
            </h2>
            <p style={{ fontSize: 15, color: C.light, fontFamily: "Georgia, serif", lineHeight: 1.7 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(280px, 1fr))", gap: 20 }}>
            {[
              { title: "Clean Room Template", desc: "A structured framework for auditing data quality before connecting systems.", tag: "Data Architecture", status: "Coming Soon", color: C.teal },
              { title: "AI Stack Navigator", desc: "How to evaluate AI tools against your operational readiness — before you buy.", tag: "Technology", status: "Coming Soon", color: C.amber },
              { title: "Logic Loop Framework", desc: "A decision-making model for operations leaders dealing with recurring system failures.", tag: "Decision Design", status: "Coming Soon", color: C.purple },
            ].map(({ title, desc, tag, status, color }) => (
              <GBox key={title} accent={color} style={{ padding: 28, opacity: 0.7 }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
                  <Pill text={tag} color={color} />
                  <span style={{ fontSize: 10, color: C.slate, fontFamily: "'Courier New', monospace", letterSpacing: 2, textTransform: "uppercase" }}>{status}</span>
                </div>
                <h3 style={{ fontSize: 16, fontWeight: 800, color: C.white, margin: "0 0 10px", fontFamily: "Georgia, serif" }}>{title}</h3>
                <p style={{ fontSize: 13, color: C.light, lineHeight: 1.7, margin: 0, fontFamily: "Georgia, serif" }}>{desc}</p>
              </GBox>
            ))}
          </div>
        </div>
      ...
      <section style={{ padding: "60px clamp(20px,6vw,80px)" }}>
        <div style={{ maxWidth: 1000, margin: "0 auto" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
            {articles.map(({ tag, date, readTime, title, preview, color }) => (
              <GBox key={title} accent={color} style={{ padding: "32px 36px" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 12 }}>
                  <Pill text={tag} color={color} />
                  <span style={{ fontSize: 12, color: C.slate, fontFamily: "Georgia, serif" }}>{date}</span>
                  <span style={{ fontSize: 12, color: C.slate, fontFamily: "Georgia, serif" }}>· {readTime}</span>
                </div>
                <h3 style={{ fontSize: "clamp(16px,2vw,22px)", fontWeight: 900, color: C.white, margin: "0 0 12px", fontFamily: "Georgia, serif" }}>{title}</h3>
                <p style={{ fontSize: 14, color: C.light, lineHeight: 1.8, margin: 0, fontFamily: "Georgia, serif" }}>{preview}</p>
              </GBox>
            ))}
          </div>
        </div>
      ...
      <section style={{ padding: "60px clamp(20px,6vw,80px)", background: C.dark, borderTop: `1px solid ${C.border}`, textAlign: "center" }}>
        <div style={{ maxWidth: 580, margin: "0 auto" }}>
          <h2 style={{ fontSize: "clamp(20px,2.5vw,32px)", fontWeight: 900, margin: "0 0 16px", fontFamily: "Georgia, serif" }}>
            Ready to go deeper than the checklist?
          </h2>
          <p style={{ fontSize: 15, color: C.light, lineHeight: 1.8, margin: "0 auto 28px", fontFamily: "Georgia, serif" }}>
