...
const pillars = [
  ...
  {
    id: 4, name: "Knowledge Architecture & Resilience", color: "#A67FF5", tag: "Resilience",
    questions: [
...
async function submitToSheets(data) {
  if (!SHEETS_WEBHOOK || SHEETS_WEBHOOK === "YOUR_APPS_SCRIPT_WEB_APP_URL_HERE") return;
  try {
...
export default function AssessmentPage({ setPage, onComplete }) {
  ...
  const calcResults = () => {
    const diagId = generateDiagnosticId();
    ...
    return { diagId, pillarScores, total, totalMax, totalPct, overallTier, tierColor, name, company, email, answers };
  };
  ...
  const handleSubmit = async () => {
    ...
    await submitToSheets({
      ...
      totalPct: results.totalPct,
      overallTier: results.overallTier,
      p1Score: results.pillarScores[0].score,
      p1Pct: results.pillarScores[0].pct,
      p1Tier: results.pillarScores[0].tier,
      p2Score: results.pillarScores[1].score,
      p2Pct: results.pillarScores[1].pct,
      p2Tier: results.pillarScores[1].tier,
      p3Score: results.pillarScores[2].score,
      p3Pct: results.pillarScores[2].pct,
      p3Tier: results.pillarScores[2].tier,
      p4Score: results.pillarScores[3].score,
      p4Pct: results.pillarScores[3].pct,
      p4Tier: results.pillarScores[3].tier,
      rawAnswers: JSON.stringify(results.answers),
    });
    setSubmitting(false);
    onComplete(results);
  };
  ...
  const inputStyle = {
    ...
    borderRadius: 6, padding: "12px 14px", color: C.white, fontSize: 15,
  ...
  if (step === 0) return (
    <div style={{ background: C.bg, minHeight: "100vh", color: C.white, display: "flex", alignItems: "center", justifyContent: "center", padding: "100px clamp(20px,5vw,60px)" }}>
      <div style={{ maxWidth: 680, width: "100%", textAlign: "center" }}>
        ...
        <h1 style={{ fontSize: "clamp(28px,4vw,48px)", fontWeight: 900, margin: "24px 0 20px", fontFamily: "Georgia, serif", lineHeight: 1.2, color: C.white }}>
        ...
        <p style={{ fontSize: 17, color: "rgba(255,255,255,0.78)", lineHeight: 1.8, margin: "0 auto 36px", fontFamily: "Georgia, serif", maxWidth: 520 }}>
        ...
        <div style={{ background: C.panel, border: "1px solid rgba(255,255,255,0.12)", borderRadius: 12, padding: 36, marginBottom: 32, textAlign: "left" }}>
          <div style={{ display: "flex", gap: 16, marginBottom: 28, flexWrap: "wrap" }}>
            {[["20", "Questions"], ["4", "Pillars"], ["425", "Point System"], ["5 min", "Estimated"]].map(([n, l]) => (
              <div key={n} style={{ flex: 1, minWidth: 80, textAlign: "center", background: "rgba(255,255,255,0.05)", borderRadius: 8, padding: "12px 8px", border: "1px solid rgba(255,255,255,0.1)" }}>
                <div style={{ fontSize: 22, fontWeight: 900, color: C.white, fontFamily: "'Courier New', monospace" }}>{n}</div>
                <div style={{ fontSize: 12, color: "rgba(255,255,255,0.55)", fontFamily: "Georgia, serif", marginTop: 4 }}>{l}</div>
              </div>
            ))}
          </div>
          <div style={{ display: "grid", gap: 12 }}>
            <div style={{ fontSize: 11, color: "rgba(255,255,255,0.5)", letterSpacing: 2.5, textTransform: "uppercase", fontFamily: "'Courier New', monospace", marginBottom: 4 }}>Your Information</div>
            <input value={name} onChange={e => setName(e.target.value)} placeholder="Your name (optional)" style={inputStyle} />
            <input value={company} onChange={e => setCompany(e.target.value)} placeholder="Company name (optional)" style={inputStyle} />
            <input value={email} onChange={e => setEmail(e.target.value)} placeholder="Email address (optional — to receive your results)" style={inputStyle} type="email" />
          </div>
        </div>
  ...
  if (step === 5) return (
    <div style={{ background: C.bg, minHeight: "100vh", color: C.white, display: "flex", alignItems: "center", justifyContent: "center", padding: "100px clamp(20px,5vw,60px)" }}>
      <div style={{ maxWidth: 600, width: "100%", textAlign: "center" }}>
        ...
        <h2 style={{ fontSize: "clamp(24px,3vw,38px)", fontWeight: 900, margin: "20px 0 16px", fontFamily: "Georgia, serif", color: C.white }}>
          {answeredCount} of {totalQuestions} questions answered
        </h2>
        <p style={{ fontSize: 16, color: "rgba(255,255,255,0.75)", lineHeight: 1.8, margin: "0 auto 36px", fontFamily: "Georgia, serif", maxWidth: 440 }}>
        ...
        <div style={{ display: "flex", gap: 16, justifyContent: "center", flexWrap: "wrap" }}>
          <button onClick={handleSubmit} disabled={submitting} style={{
            ...
            boxShadow: `0 0 30px ${C.purple}44`, opacity: submitting ? 0.7 : 1,
          }}>{submitting ? "Generating..." : "Generate My Report →"}</button>
          <button onClick={() => setStep(4)} style={{
            background: "transparent", border: "1px solid rgba(255,255,255,0.2)", borderRadius: 8, padding: "14px 28px",
            color: "rgba(255,255,255,0.82)", fontSize: 13, cursor: "pointer", fontFamily: "'Courier New', monospace", letterSpacing: 1,
      ...
      <div style={{ maxWidth: 760, margin: "0 auto" }}>
        ...
        <div style={{ marginBottom: 40 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
            <span style={{ fontSize: 13, color: "rgba(255,255,255,0.7)", fontFamily: "'Courier New', monospace", letterSpacing: 2 }}>
              PILLAR {step} OF 4
            </span>
            <span style={{ fontSize: 13, color: "rgba(255,255,255,0.7)", fontFamily: "'Courier New', monospace" }}>{answeredCount}/{totalQuestions} answered</span>
          </div>
          <div style={{ height: 5, background: "rgba(255,255,255,0.1)", borderRadius: 3 }}>
            <div style={{ height: "100%", width: `${progress}%`, background: `linear-gradient(90deg, ${C.purple}, ${C.teal})`, borderRadius: 3, transition: "width 0.4s" }} />
          </div>
          <div style={{ display: "flex", gap: 6, marginTop: 10 }}>
            {pillars.map((p, i) => (
              <div key={p.id} style={{ flex: 1, height: 3, borderRadius: 2, background: i < step ? p.color : "rgba(255,255,255,0.1)", transition: "background 0.3s" }} />
            ))}
          </div>
        </div>
        ...
        <div style={{ marginBottom: 32 }}>
          ...
          <h2 style={{ fontSize: "clamp(20px,3vw,32px)", fontWeight: 900, margin: "16px 0 8px", fontFamily: "Georgia, serif", color: C.white }}>
            {currentPillar.name}
          </h2>
          <p style={{ fontSize: 14, color: "rgba(255,255,255,0.6)", fontFamily: "'Courier New', monospace", letterSpacing: 1, margin: 0 }}>
            Rate each statement 1–5 based on your current operation
          </p>
        </div>

        {/* Scale Legend */}
        <div style={{ background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 8, padding: "14px 20px", marginBottom: 28, display: "flex", gap: 12, flexWrap: "wrap" }}>
          {scale.map(({ val, label }) => (
            <div key={val} style={{ display: "flex", alignItems: "center", gap: 6 }}>
              <span style={{ fontSize: 14, fontWeight: 700, color: currentPillar.color, fontFamily: "'Courier New', monospace", minWidth: 12 }}>{val}</span>
              <span style={{ fontSize: 12, color: "rgba(255,255,255,0.65)", fontFamily: "Georgia, serif" }}>{label}</span>
            </div>
          ))}
        </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 16, marginBottom: 40 }}>
          {currentPillar.questions.map((q, qi) => (
            <div key={q.id} style={{
              background: answers[q.id] ? "rgba(255,255,255,0.06)" : C.panel,
              border: `1px solid ${answers[q.id] ? currentPillar.color + "66" : "rgba(255,255,255,0.12)"}`,
              borderRadius: 10, padding: "24px 28px",
              boxShadow: answers[q.id] ? `0 0 20px ${currentPillar.color}18` : "none",
              transition: "all 0.2s",
            }}>
              <div style={{ display: "flex", gap: 12, alignItems: "flex-start", marginBottom: 18 }}>
                <span style={{ fontSize: 12, color: currentPillar.color, fontFamily: "'Courier New', monospace", letterSpacing: 2, flexShrink: 0, marginTop: 3, fontWeight: 700 }}>Q{q.id}</span>
                <p style={{ fontSize: 16, color: "rgba(255,255,255,0.92)", margin: 0, lineHeight: 1.7, fontFamily: "Georgia, serif" }}>{q.text}</p>
              </div>
              <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
                {[1, 2, 3, 4, 5].map(val => (
                  <button key={val} onClick={() => setAnswers(a => ({ ...a, [q.id]: val }))} style={{
                    width: 48, height: 48, borderRadius: 8,
                    background: answers[q.id] === val ? currentPillar.color : "rgba(255,255,255,0.06)",
                    border: `1px solid ${answers[q.id] === val ? currentPillar.color : "rgba(255,255,255,0.18)"}`,
                    color: answers[q.id] === val ? "#fff" : "rgba(255,255,255,0.75)",
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <button onClick={() => setStep(s => Math.max(0, s - 1))} style={{
            background: "transparent", border: "1px solid rgba(255,255,255,0.2)", borderRadius: 8, padding: "12px 24px",
            color: "rgba(255,255,255,0.82)", fontSize: 12, cursor: "pointer", fontFamily: "'Courier New', monospace", letterSpacing: 1,
          }}>← Back</button>
          <div style={{ textAlign: "center" }}>
            {!pillarComplete && (
              <p style={{ fontSize: 13, color: "rgba(255,255,255,0.5)", margin: 0, fontFamily: "Georgia, serif" }}>
          ...
          <button onClick={() => setStep(s => s + 1)} disabled={!pillarComplete} style={{
            background: pillarComplete ? `linear-gradient(135deg, ${C.purple}, ${C.teal})` : "rgba(255,255,255,0.08)",
            border: "none", borderRadius: 8, padding: "12px 28px",
            color: pillarComplete ? "#fff" : "rgba(255,255,255,0.35)",
